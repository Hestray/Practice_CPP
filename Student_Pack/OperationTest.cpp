...
void OperationTest::RUN_TEST() {
	...
	if (	ASSERT_EQUAL("[INTEGER SUM TEST, (int)BasicOp::sum]", BasicOp::sum(numbers), 9)					&&
			...
			ASSERT_EQUAL("[INTEGER DIV TEST (1), (int)BasicOp::div]", BasicOp::div(2, 0), -1)				&&
			ASSERT_EQUAL("[INTEGER DIV TEST (2), (int)BasicOp::div]", BasicOp::div(5, 2), 2)				&&
			ASSERT_EQUAL("[INTEGER POW TEST, (int)BasicOp::pow]", BasicOp::pow(2, 4), 16)					
			) {
